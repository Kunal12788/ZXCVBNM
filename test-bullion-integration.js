...
async function runTest() {
  ...
  if (customers.length === 0) {
    ...
    console.log('✅ Test customer already exists. Resetting last sent timestamps so they are due...');
    const { error: resetError } = await supabase
      .from('bullion_whatsapp_customers')
      .update({
        last_notification_sent_at: new Date(Date.now() - 60 * 60 * 1000).toISOString() // 1 hour ago
      })
      .eq('id', customerId);
